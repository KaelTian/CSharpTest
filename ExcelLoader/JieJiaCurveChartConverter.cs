...
namespace ExcelLoader
{
    ...
    /// 捷佳曲线图数据映射配置
    /// </summary>
    public class JieJiaCurveChartConfig
    {
        /// <summary>
        /// 分组类型
        /// </summary>
    ...
    /// 捷佳曲线图转换器
    /// </summary>
    public class JieJiaCurveChartConverter
    {
        /// <summary>
        ...
        public void ParseIndexMappingFromExcel(string excelFilePath, string outputFilePath)
        {
            try
            {
                var configs = GetJieJiaCurveChartConfigs(excelFilePath);
        ...
        public void GenerateTableScript(string excelPath, string dbName, string tableName, string outputPath)
        {
            ...
            var configs = GetJieJiaCurveChartConfigs(excelPath);
        ...
        private List<JieJiaCurveChartConfig> GetJieJiaCurveChartConfigs(string excelFilePath)
        {
            var configs = new List<JieJiaCurveChartConfig>();
            ...
            using (var package = new ExcelPackage(new FileInfo(excelFilePath)))
            {
                ...
                for (int row = startRow; row <= rowCount; row++)
                {
                    ...
                    string description = worksheet.Cells[row, 10].Text;
                    string englishName = worksheet.Cells[row, 11].Text;
                    ...
                    configs.Add(new JieJiaCurveChartConfig
                    {
                        Id = plcTag,
                        PlcTag = plcTag,
            ...
            configs = configs.OrderBy(config => config.Category).ThenBy(config => config.Index).ToList();
            return configs;
        }
